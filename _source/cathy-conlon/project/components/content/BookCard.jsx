...
export function BookCard({ title, year, publisher, kind, cover, blurb, href, layout = "column", featured = false, tag, actions, className, ...rest }) {
  const Tag = href ? "a" : "article";
  return <Tag href={href} className={cx("ih-book", layout === "row" && "ih-book--row", featured && "ih-book--featured", className)} {...rest}>
    <div className="ih-book__cover">{cover ? <img src={cover} alt={`Cover of ${title}`} /> : <div className="ih-book__placeholder">{title}</div>}{tag && <span className="ih-badge ih-badge--inverse ih-book__tag">{tag}</span>}</div>
