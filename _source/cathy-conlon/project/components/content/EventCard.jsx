...
export function EventCard({ date, title, venue, city, time, kind, href, past = false, showYear = false, soldOut = false, layout = "row", actions, className, ...rest }) {
  ...
  return <Tag href={href} className={cx("ih-event", past && "ih-event--past", layout === "card" && "ih-event--card", className)} {...rest}>
    <time className="ih-event__date" dateTime={iso}><span className="ih-event__day">{day}</span><span className="ih-event__month">{month}{showYear && <> {d.getFullYear()}</>}</span></time>
