...
- `src/pages/` — `Home`, `Service` (template for alarms, CCTV, fire), `Survey` (3-step request flow).
- `src/services.ts` — content for the three service pages.
- `src/router.ts` — hash routes: `#/`, `#/cctv`, `#/survey`, `#/alarms`, `#/fire`, plus home sections `#/services` `#/reviews` `#/questions`.
- `src/components/CoveragePlan.tsx` — plan-view drawings: camera views, alarm sensors, fire detectors (home and business examples each).
...
- Confirm with Amptech: urgent-call promise in the top strip; alarm and fire page copy (grades, install times, servicing) in `src/services.ts`; alarm and fire guide prices if they want a pricing section
