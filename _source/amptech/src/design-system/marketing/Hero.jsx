...
export function Hero({ eyebrow, title, description, primaryCta = "Request a survey", secondaryCta = "Call us", onPrimary, onSecondary, media, proof = [], inverse = false, style }) {
  ...
  return <section style={{ background: inverse ? "var(--surface-brand)" : "var(--surface-page)", color: fg, ...style }}>
    <div className="ds-split" style={{ "--cols": media ? "minmax(0, 7fr) minmax(0, 5fr)" : "minmax(0, 8fr) minmax(0, 4fr)", "--split-gap": "var(--space-9)", "--split-align": "center", maxWidth: "var(--container-max)", margin: "0 auto", padding: "clamp(48px, 8vw, var(--space-11)) var(--grid-margin)" }}>
      ...
      {media && <div className="ds-hero-media" style={{ borderRadius: "var(--radius-xl)", overflow: "hidden", background: inverse ? "rgba(255,255,255,0.08)" : "var(--surface-muted)" }}>{media}</div>}
    </div>
  </section>;
}
