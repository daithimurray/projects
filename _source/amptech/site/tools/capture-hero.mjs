...
//   node tools/capture-hero.mjs [--port 4411] [--quality 0.85] [--og-quality 85]
//
...
// framed at that size's own aspect. Writes to public/:
//   hero/poster-tall.webp    900x1500  (0.6, phones and portrait tablets)
//   hero/poster-square.webp 1200x1200  (1.0, squarish windows, foldables)
//   hero/poster-wide.webp   1600x1000  (1.6, laptops and most desktops)
//   hero/poster-ultra.webp  2100x1000  (2.1, wide windows, landscape phones)
//   og.jpg                  1200x630   (the hero with its headline, JPEG q85)
// The live scene frames itself like whichever poster is on screen, so if you
// change a size here, change the matching <source> in Hero.astro too.
...
const ogQuality = Number(opt('og-quality', 85));
...
const POSTERS = [
  ...
  ['poster-square', 1200, 1200],
  ['poster-wide', 1600, 1000],
  ['poster-ultra', 2100, 1000],
];
...
// Social card: the hero as a visitor sees it, headline included. JPEG keeps it
// well under the ~300 KB that link-preview crawlers (WhatsApp) will fetch.
{
  ...
  const file = out('og.jpg');
  await page.screenshot({ path: file, type: 'jpeg', quality: ogQuality, clip: { x: 0, y: 0, width: 1200, height: 630 } });
